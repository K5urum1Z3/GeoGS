...
class GaussianModel:
    ...
    def restore(self, model_args, training_args):
        ...
        self._load_deformation_state_dict(deform_state, source_name="restore()")
        ...
        try:
            ...
            print("Warning: optimizer state_dict is incompatible with current parameter groups, reset optimizer state.")
            print(exc)
    ...
    def training_setup(self, training_args):
        ...
        self.geodesic_sphere_scheduler_args = None
        self.geodesic_hyper_scheduler_args = None

        geo_lr_default = 0.000025
        geo_lr_final_default = 0.0000025
        geo_lr = getattr(training_args, "geo_lr", geo_lr_default)
        geo_lr_sphere = getattr(training_args, "geo_lr_sphere", geo_lr_default)
        geo_lr_hyper = getattr(training_args, "geo_lr_hyper", geo_lr_default)
        geo_lr_sphere_final = getattr(training_args, "geo_lr_sphere_final", geo_lr_final_default)
        geo_lr_hyper_final = getattr(training_args, "geo_lr_hyper_final", geo_lr_final_default)
        geo_lr_delay_mult = getattr(training_args, "geo_lr_delay_mult", training_args.deformation_lr_delay_mult)

        if abs(geo_lr_sphere - geo_lr_default) < 1e-12 and abs(geo_lr - geo_lr_default) > 1e-12:
            geo_lr_sphere = geo_lr
        if abs(geo_lr_hyper - geo_lr_default) < 1e-12 and abs(geo_lr - geo_lr_default) > 1e-12:
            geo_lr_hyper = geo_lr
        if abs(geo_lr_sphere_final - geo_lr_final_default) < 1e-12 and abs(geo_lr_sphere - geo_lr_default) > 1e-12:
            geo_lr_sphere_final = geo_lr_sphere * (geo_lr_final_default / geo_lr_default)
        if abs(geo_lr_hyper_final - geo_lr_final_default) < 1e-12 and abs(geo_lr_hyper - geo_lr_default) > 1e-12:
            geo_lr_hyper_final = geo_lr_hyper * (geo_lr_final_default / geo_lr_default)

        geodesic_sphere_params = list(self._deformation.get_geodesic_sphere_parameters())
        if len(geodesic_sphere_params) > 0:
            l.append(
                {
                    'params': geodesic_sphere_params,
                    'lr': geo_lr_sphere * self.spatial_lr_scale,
                    "name": "geodesic_motion_sphere",
                }
            )
            self.geodesic_sphere_scheduler_args = get_expon_lr_func(
                lr_init=geo_lr_sphere * self.spatial_lr_scale,
                lr_final=geo_lr_sphere_final * self.spatial_lr_scale,
                lr_delay_mult=geo_lr_delay_mult,
                max_steps=training_args.position_lr_max_steps,
            )

        geodesic_hyper_params = list(self._deformation.get_geodesic_hyper_parameters())
        if len(geodesic_hyper_params) > 0:
            l.append(
                {
                    'params': geodesic_hyper_params,
                    'lr': geo_lr_hyper * self.spatial_lr_scale,
                    "name": "geodesic_motion_hyper",
                }
            )
            self.geodesic_hyper_scheduler_args = get_expon_lr_func(
                lr_init=geo_lr_hyper * self.spatial_lr_scale,
                lr_final=geo_lr_hyper_final * self.spatial_lr_scale,
                lr_delay_mult=geo_lr_delay_mult,
                max_steps=training_args.position_lr_max_steps,
            )
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if group_name == "xyz":
                ...
                param_group['lr'] = lr
            elif "grid" in group_name:
                lr = self.grid_scheduler_args(iteration)
                param_group['lr'] = lr
            elif group_name == "deformation":
                lr = self.deformation_scheduler_args(iteration)
                param_group['lr'] = lr
            elif group_name == "geodesic_motion_sphere" and self.geodesic_sphere_scheduler_args is not None:
                lr = self.geodesic_sphere_scheduler_args(iteration)
                param_group['lr'] = lr
            elif group_name == "geodesic_motion_hyper" and self.geodesic_hyper_scheduler_args is not None:
                lr = self.geodesic_hyper_scheduler_args(iteration)
                param_group['lr'] = lr
    ...
    def load_model(self, path):
        ...
        self._load_deformation_state_dict(weight_dict, source_name="load_model()")
        self._deformation = self._deformation.to("cuda")
        self._deformation_table = torch.gt(torch.ones((self.get_xyz.shape[0]),device="cuda"),0)
        self._deformation_accum = torch.zeros((self.get_xyz.shape[0],3),device="cuda")
    ...
    def _load_deformation_state_dict(self, state_dict, source_name="state_dict"):
        incompatible = self._deformation.load_state_dict(state_dict, strict=False)
        if len(incompatible.missing_keys) > 0 or len(incompatible.unexpected_keys) > 0:
            print("Warning: deformation {} loaded with non-strict key match.".format(source_name))
            if len(incompatible.missing_keys) > 0:
                print("Missing keys: {}".format(incompatible.missing_keys))
            if len(incompatible.unexpected_keys) > 0:
                print("Unexpected keys: {}".format(incompatible.unexpected_keys))
